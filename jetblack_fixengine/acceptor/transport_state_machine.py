...
class AcceptorTransportStateMachine(TransportStateMachineAsync):
    ...
    async def _handle_fix(
        ...
        await self.acceptor.session.save_message(transport_message.buffer)

        fix_message = self.acceptor.fix_message_factory.decode(
        ...
        if msgcat == 'admin':
            ...
            await self.acceptor.on_application_message(fix_message.message)

        msg_seq_num: int = cast(int, fix_message.message['MsgSeqNum'])
        await self.acceptor.session.set_incoming_seqnum(msg_seq_num)
    ...
    async def _handle_admin_message(self, message: Mapping[str, Any]) -> None:
        ...
        await self.acceptor.on_admin_message(message)
    ...
    async def _handle_timeout(
        ...
        elapsed = seconds_since_last_receive - self.acceptor.heartbeat_timeout
        if elapsed > self.acceptor.heartbeat_threshold:
            await self._admin_state_machine.process(
